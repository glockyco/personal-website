const employment = {
  title: 'Researcher and Senior Software Engineer',
  organization: {
    name: 'Software Competence Center Hagenberg',
    shortName: 'SCCH',
    url: 'https://www.scch.at/'
  },
  group: {
    name: 'AI-based Engineering of Sustainable Systems',
    shortName: 'AISYS',
    url: 'https://www.scch.at/software-science/projekte/detail/aisys'
  },
  location: 'Hagenberg, Austria',
  startYear: 2026
} as const;

const doctorate = {
  candidateTitle: 'PhD candidate in Informatics',
  degree: 'Dr. techn. (PhD)',
  field: 'Informatics',
  institution: {
    name: 'University of Klagenfurt',
    url: 'https://www.aau.at/en/'
  },
  group: {
    name: 'Software Engineering Research Group',
    shortName: 'SERG',
    url: 'https://www.aau.at/en/isys/serg/'
  },
  location: 'Klagenfurt, Austria',
  startYear: 2021,
  advisors: ['Prof. Martin Pinzger', 'Prof. Johann Eder'],
  expectedGraduation: 'Q4 2026'
} as const;

export const profile = {
  name: 'Johann Glock',
  tagline: 'Researcher, Senior Software Engineer, PhD Candidate',
  location: 'Hagenberg / Linz, Austria',
  employment,
  doctorate,
  orcid: '0000-0002-0152-8611',
  github: 'https://github.com/glockyco',
  scholar: 'https://scholar.google.com/citations?user=33UUDmcAAAAJ',
  semanticScholar: 'https://www.semanticscholar.org/author/Johann-Glock/2266466770',
  linkedin: 'https://www.linkedin.com/in/johann-glock-486a9b221/',

  bioHtml: `Johann Glock is a researcher and senior software engineer in the <a href="${employment.group.url}">${employment.group.shortName} group</a> at the <a href="${employment.organization.url}">${employment.organization.name} (${employment.organization.shortName})</a>. He is also a PhD candidate in Informatics at the <a href="${doctorate.institution.url}">${doctorate.institution.name}</a>, supervised by ${doctorate.advisors.join(' and ')}. His doctoral research focuses on practical applications of formal program analysis, including semantic differencing and software test generalization. He developed <a href="/research/pasda/">PASDA</a> and <a href="/research/teralizer/">Teralizer</a>. Outside his professional work, he data-mines games, builds interactive maps and mods, and contributes tools to their communities.`
} as const;
